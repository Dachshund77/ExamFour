...
//Update a User 
router.put('/:_id', [ //TODO useer should be able to change his records, but only certain values
    requireAuthentication,
    authorizeUserWithRoles(['Admin']),
    requireAuthorisation,
    isValidObjectID
    ...
    try {
        ...
        User.findOneAndUpdate({ _id: req.params._id }, req.body, { new: true }, function(err, doc) {
            if (err) {
                if (err.code === 11000) {
                    //duplicate key
                    res.status(400).json(responses.badRequest("Key exists already in database", err));
...
router.delete('/:_id', [
    ...
    try {
        //delete
        await User.deleteOne({ _id: req.params._id }, function(err, doc) {
            if (err) {
                if (err instanceof mongoose.Error.ValidationError) {
                    res.status(400).json(responses.badRequest("Validation failed for request", err));
                    return
...
router.get('/:name', [
    ...
    try {
        ...
        await User.findOne({ 'name': req.params.name }, function(err, doc) {
            if (err) {
                if (err instanceof mongoose.Error.ValidationError) {
                    res.status(400).json(responses.badRequest("Validation failed for request", err));
                    return
